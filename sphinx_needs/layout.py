...
def create_need(
    ...
    node_container[0].children[0].attributes = (
        node_container.parent.children[0].children[0].attributes
    )
...
class LayoutHandler:
    ...
    def __init__(
        ...
        self.functions: Dict[
            str, Callable[..., Union[None, nodes.Node, List[nodes.Node]]]
        ] = {
            "meta": self.meta,
    ...
    def meta_links(self, name: str, incoming: bool = False) -> nodes.inline:
        ...
        if incoming:
            node_links = NeedIncoming(
                reftarget=self.need["id"], link_type=f"{name}_back"
            )
        else:
    ...
    def _grid_simple(
        self,
        colwidths: List[int],
        side_left: Union[bool, str],
        side_right: Union[bool, str],
        footer: bool,
    ) -> None:
        """
